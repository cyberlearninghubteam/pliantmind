      ...
      <div className="min-h-screen pt-28 pb-20 px-6">
        <div className="mx-auto max-w-2xl">
            ...
            <div className="space-y-4">
              {recommendedTracks.map((track) => track && (
                <Link
                  ...
                  className="card rounded-lg p-6 flex items-start gap-4 hover:border-accent/30 transition-all block"
                >
          ...
          <div className="text-center space-y-3">
            ...
            <button
              onClick={() => { setAnswers([]); setCurrentQuestion(0); setResult(null); setStarted(false); }}
              className="block mx-auto text-sm text-text-muted hover:text-text-primary transition-colors"
            >
              Retake assessment
            </button>
          </div>
          ...
          <div className="space-y-3">
            {q.options.map((option, i) => (
              <button
                ...
                className="w-full text-left card rounded-lg p-5 hover:border-accent/40 hover:bg-surface-warm transition-all group"
              >
                <span className="text-[15px] text-text-primary group-hover:text-accent transition-colors">
                  {option.text}
                </span>
              </button>
            ))}
